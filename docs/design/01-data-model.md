...
> v0.3 变更：新接入账号的地址若已是别人，并入你并重算方向。
...
- **重新派生不是版本。** 版本记录的是上游的变化，它以一条新的 Raw 到来。同一条 Raw 用更好的规范化重新派生（更好的 HTML 转文本、更准的线程归属），是核心改进了自己的理解，不是对方改了什么，所以原地更新那条 Item：文本、人、会话、附件重新取自字节，id、来源、级别、墓碑不动。一条 Raw 永远只对应一条 Item。
...
接第二个、第三个账号时，这个地址往往早已作为"别人"出现过：两个邮箱互相发过信。接入即是你的断言，所以那个 Person 并入你：它写的和收的每条 Item、两个方向的承诺都改挂到你身上，direction 按新的身份重算，被并入的 id 记进 merged_from，可拆。
