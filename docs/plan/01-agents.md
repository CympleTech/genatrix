...
| A3 | done: `Effect::Agent` with a data-only card; not editable; approved, it is handed out like a connector's action and carried out by the core calling the agent's `apply` inside a savepoint, so it lands whole or not at all, and only by the version that proposed it; carried out right after approval and on the loop. Outward mail from an agent is `{to, subject, body, reply_to?}` checked against the manifest's targets (a fixed list, the same thread for an item read in this run, or people the user has written to) before anything is stored. The manifest's daily proposal count and the core's ceiling pause the agent when reached; pending proposals stay for the user to see. The approval card draws agent cards with one fixed template. Tests for invariants 16 and 21 and ruling 8. Agents may not yet propose chat messages or events |
| A4 | done: Settings → Agents: install from a file on this machine (read without running, the grants in the reader's language, a risk tier, a trial on the five most recent items in scope with a throwaway space and proposals held back), pause (which withdraws what it proposed), resume, export its space as JSON, uninstall (deletes its packages and space; the ledger keeps what it did). Agents are parties in Chats: counts, grants, a conversation of what you said, what woke it, what it answered, and its proposals as approval cards; writing to it runs it. Triggers: new items in scope handed over in storage order from a cursor that starts at install, and schedules (daily, weekly, monthly, local time), checked on the loop. The menu bar window opens a file panel and hands downloads to the browser. Test for invariant 18. The dry run on a snapshot is the trial on live data read-only; design 11's wording stands |
